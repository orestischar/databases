...
<?php
	...
	if(isset($_GET['Customer_IRS_Number']) && isset($_GET['Hotel_ID']) && isset($_GET['Room_ID']) && isset($_GET['Start_Date']) && isset($_GET['Finish_Date']) )
	{	
		...
		header("Location: InsertRents.php#popup3");
	...
	<?php require 'connectToDB.php'; ?>
		...
		<div id = "popup1" class="overlay">
			<div class="popup">
			    ...
			    ?>
				<h2 style = "font-size:18px; text-align: justify;">Choose an existing Reservation - based on Customer IRS number,Hotel ID, and Room ID - , erase it and convert it into Rent by filling the employee responsible and pressing Submit!</h2><br>
				<a class="close" href="index.php"><button type="button" id="delete">X</button></a>
				<div class="content">

				<nav class="dropdown">
					<button onclick="myFunction()" class="dropbtn">Choose a checked-in Reservation</button>
					    ...
					    <input type="text" placeholder="Search..." id="myInput" onkeyup="filterFunction()">
					    	...
					    	<?php
								...
								{	
									...
									echo ('<tr><td style="background-color : #808080;"><a href="InsertRents.php?Customer_IRS_Number=' . $row['Customer_IRS_Number']. '&Hotel_ID=' . $row['Hotel_ID']. '&Room_ID=' . $row['Room_ID']. '&Start_Date=' . $row['Start_Date']. '&Finish_Date=' . $row['Finish_Date'].'">' . $row['Customer_IRS_Number'] .'</td><td style="background-color : #808080;">'. $row['Hotel_ID'] .'</td><td style="background-color : #808080;">'. $row['Room_ID'] .'</td><td>' .$row['First_Name'] .'</td><td>'. $row['Last_Name'] .'</td><td>' . $row['Start_Date'] .'</td><td>'. $row['Finish_Date'] . '</td><tr>');
					...
					<input type="int" name="Hotel_ID" value="<?php echo $Hotel_ID;?>" disabled><br>

					<label for="Room_ID">Room ID 🔒</label><br>
					<input type="int" name="Room_ID" value="<?php echo $Room_ID;?>" disabled><br>

					<label for="Start_Date">Start Date 🔒</label><br>
					<input type="date" name="Start_Date" value="<?php echo $Start_Date;?>" disabled><br>

					<label for="Finish_Date">Finish Date 🔒</label><br>
					<input type="date" name="Finish_Date" value="<?php echo $Finish_Date;?>" disabled><br>

					<label for="Employee_IRS_Number">Employee_IRS_Number: </label><br>
					<input  type="int" name="Employee_IRS_Number" placeholder="Employee responsible..." value="<?php echo $Employee_IRS_Number;?>" disabled><br>
		...
		<div id="popup3" class="overlay">
			<div class="popup">
				<div class = "content">
					<h2>Fill the employee responsible and press Submit to confirm the convertion!</h2><br><br><br>
